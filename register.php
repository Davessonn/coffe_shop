...
if(isset($_POST['submit']))
{
    ...
    if(!empty($username) && !empty($password1) && !empty($password2) && ($password1 == $password2))
    {
        ...
        if(mysqli_num_rows($data) == 0)
        {
            ...
            echo '<h2><a href = "index.php">&lt;&lt;Back to the main page</a></h2>';
            mysqli_close($dbc);
            exit();
        }else{ echo '<h3>There is an account for this username, plesase use a different one.</h3>'; $username = ""; }

    }else{ echo '<h3>Each data has to be entered (password twice).</h3>';}
